...
/**
 * The code that runs during lazy image loading.
 */
function oxi_addons_public_imagesloaded() {
    wp_enqueue_script('oxi-imagesloaded', plugins_url('jquery/imagesloaded.pkgd.min.js', __FILE__));
}

/**
 * The code that runs during asProgress js loading.
 */
function oxi_addons_public_asProgress() {
    wp_enqueue_script('oxi-asProgress', plugins_url('jquery/jquery-asProgress.min.js', __FILE__));
}

/**
 * The code that runs during counterup js loading.
 */
function oxi_addons_public_counterup() {
    wp_enqueue_script('counterup-min', plugins_url('jquery/jquery-counterup-min.js', __FILE__));
}

/**
 * The code that runs during isotope js loading.
 */
function oxi_addons_public_isotope() {
    wp_enqueue_script('oxi-isotope-min', plugins_url('jquery/jquery.isotope.v3.0.2.js', __FILE__));
}

/**
 * The code that runs during countdown js loading.
 */
function oxi_addons_public_countdown() {
    wp_enqueue_script('jquery.countdown.min', plugins_url('jquery/jquery.countdown.min.js', __FILE__));
}
 ...
 */
function oxi_addons_owl_carousel() {
    wp_enqueue_style('oxi-addons-owl-carousel', plugins_url('css/owl.carousel.css', __FILE__));
    wp_enqueue_script('oxi-addons-owl-carousel', plugins_url('jquery/owl.carousel.js', __FILE__));
}
...
$SmoothScrolling = get_option('oxi_addons_smooth_scrolling');
if ($SmoothScrolling == 'yes') {
    if (!is_admin()) {
        add_action('wp_head', 'OxiAddonsSmoothScrollingPagePost');
    }
}

function OxiAddonsSmoothScrollingPagePost() {
    $SmoothScrollingPage = get_option('oxi_addons_smooth_scrolling_page');
    $SmoothScrollingPost = get_option('oxi_addons_smooth_scrolling_post');
    $SmoothScrollingOffset = get_option('oxi_addons_smooth_scrolling_offset');
    $jquery = 'setTimeout(function () {jQuery("a[href*=\'#\']").not("[href=\'#\']").not("[href=\'#0\']").click(function (event) {
                    if (
                        ...
                        if (target.length) {
                            ...
                            $("html, body").animate({
                                scrollTop: target.offset().top + ' . $SmoothScrollingOffset . '
                            }, 1000);
                        }
                    }
                });}, 500);';
    if (is_page('' && $SmoothScrollingPage == 'yes')) {
        oxi_addons_public_style();
        wp_add_inline_script('oxi-addons-animation', $jquery);
    }
    if (get_post_type('') && $SmoothScrollingPost == 'yes') {
        oxi_addons_public_style();
        wp_add_inline_script('oxi-addons-animation', $jquery);
    }
}
